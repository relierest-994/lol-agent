...
import { getChampionAvatarUrl, getOutcomeColor, getOutcomeLabel, formatKda } from '../mobile-ui-model';
...
function heroChangeTagMeta(tag: ChangeTag): { label: string; color: string; bg: string } {
...
export function AppInnerShell(props: { session: AppUserSession; onLogout: () => void; styles: Record<string, any> }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!alive) return;
        setFeedback({ title: '加载失败', message: error instanceof Error ? error.message : '首页数据加载失败' });
      } finally {
  ...
  async function handleBind(): Promise<void> {
    ...
    if (!ok) setBindModalVisible(true);
  }
  ...
  function renderHome(): React.ReactElement {
    return (
      <ScrollView contentContainerStyle={props.styles.pageBody}>
        <View style={props.styles.card}>
          ...
          {(homeData?.updates ?? []).map((item) => (
            <View key={item.id} style={props.styles.listItemCard}>
              <Text style={props.styles.itemTitle}>{item.title}</Text>
              <Text style={props.styles.itemSub}>Patch {item.patch}</Text>
              <Text style={props.styles.itemDesc}>{item.detail}</Text>
            </View>
          ))}
  ...
  function renderDataCenter(): React.ReactElement {
    ...
    return (
      <ScrollView contentContainerStyle={props.styles.pageBody}>
        ...
        <TrendBars styles={props.styles} title="近局KDA趋势" values={dataCenter?.charts.kdaTrend ?? []} color="#28A06C" />
    ...
    return (
      <ScrollView contentContainerStyle={props.styles.pageBody}>
        <View style={props.styles.card}>
          ...
          <View style={props.styles.row}>
            <GhostButton styles={props.styles} title="英雄" onPress={() => setHeroMetaTab('HERO')} />
            <GhostButton styles={props.styles} title="天赋" onPress={() => setHeroMetaTab('RUNE')} />
            <GhostButton styles={props.styles} title="装备" onPress={() => setHeroMetaTab('ITEM')} />
          </View>
          <View style={props.styles.row}>
            {changeOptions.map((item) => (
              <Pressable key={`chg-${item}`} style={[props.styles.positionChip, changeTag === item ? props.styles.positionChipActive : undefined]} onPress={() => setChangeTag(item)}>
                <Text style={[props.styles.positionChipText, changeTag === item ? props.styles.positionChipTextActive : undefined]}>{item === 'ALL' ? '全部' : heroChangeTagMeta(item).label}</Text>
              </Pressable>
            ))}
          </View>
          {heroMetaTab === 'HERO' ? (
            <View style={props.styles.row}>
              {positionOptions.map((item) => (
                <Pressable key={item} style={[props.styles.positionChip, heroPosition === item ? props.styles.positionChipActive : undefined]} onPress={() => setHeroPosition(item)}>
                  <Text style={[props.styles.positionChipText, heroPosition === item ? props.styles.positionChipTextActive : undefined]}>{item === 'ALL' ? '全部' : item}</Text>
                </Pressable>
              ))}
            </View>
          ) : null}
  ...
  function renderMine(): React.ReactElement {
    return (
      <ScrollView contentContainerStyle={props.styles.pageBody}>
        <View style={props.styles.card}>
          <Text style={props.styles.sectionTitle}>我的账号</Text>
          <View style={props.styles.profileHead}>
            <Image source={{ uri: props.session.avatarUrl }} style={props.styles.profileAvatar} />
            <View style={props.styles.profileBody}>
              <Text style={props.styles.profileName}>{props.session.nickname ?? '未设置昵称'}</Text>
              <Text style={props.styles.profileText}>手机号：{props.session.phone}</Text>
              <Text style={props.styles.profileText}>游戏账号：{linked ? vm.accountLabel : '未绑定'}</Text>
            </View>
          </View>
          <View style={props.styles.row}>
            <GhostButton styles={props.styles} title="绑定账号" onPress={() => setBindModalVisible(true)} />
            <GhostButton styles={props.styles} title="退出登录" onPress={props.onLogout} />
          </View>
        </View>
        <View style={props.styles.card}>
          <View style={props.styles.rowBetween}>
            <Text style={props.styles.sectionTitle}>战绩列表</Text>
            <GhostButton styles={props.styles} title="刷新" onPress={() => void vm.refreshRecentMatches()} disabled={!linked || vm.loadingRecentMatches} />
          </View>
          {!linked ? <View style={props.styles.emptyBox}><Text style={props.styles.emptyTitle}>请先绑定游戏账号</Text><Text style={props.styles.emptyDesc}>绑定后可查看战绩并生成复盘报告。</Text><PrimaryButton styles={props.styles} title="去绑定" onPress={() => setBindModalVisible(true)} /></View> : null}
          {linked && vm.recentMatches.length === 0 ? <View style={props.styles.emptyBox}><Text style={props.styles.emptyTitle}>暂无战绩</Text><Text style={props.styles.emptyDesc}>当前账号无可展示对局数据。</Text></View> : null}
          {linked ? vm.recentMatches.map((item) => <MatchRow styles={props.styles} key={item.matchId} item={item} selected={vm.selectedMatchId === item.matchId} onSelect={() => vm.setSelectedMatchId(item.matchId)} onReview={() => void startBasicReview(item.matchId)} onCoach={() => openCoach(item.matchId)} onOpenDetail={() => void openMatchDetail(item.matchId)} />) : null}
        </View>
      </ScrollView>
    );
  }
  ...
  return (
    <SafeAreaView style={props.styles.safe}>
      ...
      {overlayPage === 'MAIN' ? <View style={props.styles.mainLayout}><View style={props.styles.mainPane}>{renderMainTabContent()}</View><MainBottomTabBar activeTab={mainTab} onChange={setMainTab} /></View> : null}
      {overlayPage === 'REVIEW' ? <ScrollView contentContainerStyle={props.styles.pageBody}><View style={props.styles.card}><View style={props.styles.rowBetween}><Text style={props.styles.sectionTitle}>复盘摘要</Text><GhostButton styles={props.styles} title="返回" onPress={() => setOverlayPage('MAIN')} /></View><Text style={props.styles.reviewText}>{reviewSummary}</Text></View></ScrollView> : null}
